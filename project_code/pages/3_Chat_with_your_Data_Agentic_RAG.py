...
st.set_page_config(
    page_title="Chat with Documents(Agentic RAG)",
    page_icon="📚",
...
if st.session_state.vector_store and not st.session_state.rag_agent:
    ...
    def grade_documents(state: AgentState):
        ...
        if not docs:
            return {
                "steps": state["steps"] + ["⚠️ No documents found"]
            }
...
Answer with just 'yes' or 'no'."""
        ...
        return {
            "steps": state["steps"] + [f"{'✅' if is_relevant else '❌'} Graded documents: {'relevant' if is_relevant else 'not relevant'}"]
        }

    # Router function to decide next step after grading
    def decide_next_step(state: AgentState) -> Literal["generate", "rewrite"]:
        """Decide whether to generate answer or rewrite question"""
        # Check the last step to see if documents were relevant
        last_step = state["steps"][-1] if state["steps"] else ""

        # Count how many times we've rewritten to prevent infinite loops
        rewrite_count = sum(1 for step in state["steps"] if "Rewrote question" in step)

        # If we've rewritten 2+ times, just generate an answer
        if rewrite_count >= 2:
            return "generate"

        # If documents are relevant, generate answer
        if "relevant" in last_step and "not relevant" not in last_step:
            return "generate"
    ...
    workflow.add_edge("retrieve", "grade")  # After retrieve, grade the documents
    workflow.add_conditional_edges("grade", decide_next_step)  # Use router to decide next step
    workflow.add_edge("rewrite", "retrieve")  # After rewrite, retrieve again
    workflow.add_edge("generate", END)

    # Compile and save with recursion limit
        ...
        with st.chat_message("assistant"):
            with st.spinner("Agent is working..."):
                ...
                result = st.session_state.rag_agent.invoke(
                    ...
                    config={"recursion_limit": 50}  # Increase limit from default 25
                )
